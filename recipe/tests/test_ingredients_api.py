...
class PublicIngredientsApiTest(TestCase):
    """ Api incredientes accesibles publicamente """
    def setUp(self):
        self.client = APIClient()
    
    def test_login_required(self):
        """ Prueba que login sea requrido para acceder a este endpoint """
        res = self.client.get(INGREDIENTS_URL)
        self.assertEqual(res.status_code, status.HTTP_401_UNAUTHORIZED)

class PrivateIngredientsApiTest(TestCase):
    """ Api incredientes accesibles en privado """
    def setUp(self):
        self.client = APIClient()
    ...
    def test_ingredients_to_user(self):
        """ Probar ingredientes autenticados solamante autenticados por el usuario """

        user2 = get_user_model().objects.create_user(email='user@user.com', password='Userpass')
