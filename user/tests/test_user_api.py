...
class PublicUserApiTests(TestCase):
    ...
    def test_user_exists(self):
        """ Probar que un usuario ya existe falla """
        payload = {
            'email':'test@datadosis.com',
            'password': 'Testpass',
            'name': 'Test name'
    ...
    def test_create_token_missing_field(self):
        """ Probar que el email y contraseña son requeridos """
        res = self.client.post(TOKEN_URL, {'email':'corro invalido', 'password':'contraseña'})
        self.assertNotIn('token', res.data)
        self.assertEqual(res.status_code, status.HTTP_400_BAD_REQUEST)

    def test_retrieve_user_unathorized(self):
        """ Prueba que la autorizacion sea requerida para los usuarios """
...
class PrivateUserApiTests(TestCase):
    ...
    def test_retrieve_profile_success(self):
        """ Probar obtener perfil de usuario con login """
    ...
    def test_update_user_profile(self):
        """ Preuba que el usuario esta siendo actualizado si esta autenticado """
        payload = {
            'password': 'NewTestopass',
            'name': 'new name'
        }
